...
func (ba *ByteArray) NextUTF() string {
	...
	for count < utflen {
		...
		case 13:
			count += 2
			if count > utflen {
				log.Println("malformed input: partial character at end")
			}
			char2 = int(bytearr[count -1])
			if char2 & 0xc0 != 0x80 {
				log.Println("malformed input around byte ", count)
			}
		...
		case 14:
			...
			if (char2 & 0xC0) != 0x80 || (char3 & 0xC0) != 0x80 {
				log.Println("malformed input around byte ", count-1)
			}
